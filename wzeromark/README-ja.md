...
| 名称 | 長さ | 目的 | 詳細 |
| :-- | --: | :--| :-- |
| バージョン | 1B | マークの構成管理のため | リトルエディアン |
| 組織コード | 2.5B | 組織識別のため | 通常5桁のHexで表現される組織ごとに固有な値 |
| ノンス | 1.5B | 外部リソースへのソース特定のため | 乱数 |
...
20bit（2.5B）の値。16進数で5桁表現（例: `0x0a1b2`）。
約104万通り（1,048,576通り）の組織を識別可能。
組織ごとにランダムに決定する。
...
この衝突は、外部リソースへのソース特定ができないことを意味するため、先のノンスによって、一意性を確保する。
